(function ($) {
  ...
  $(document).on("click", ".add-btn", function () {
    ...
    // Quand un élément est sélectionné dans la liste des suggestions
    $(document).on("click", ".suggestion-item", function () {
      ...
      row.find(".item-price").val(itemPrice); // Remplir le prix
      ...
      if (quantity) {
        ...
        row.find(".item-amount").val(amount); // Remplir le montant
      }

      row.find("#item-suggestions").hide(); // Cacher les suggestions après sélection
    });

    // Ajouter un calcul automatique du montant quand la quantité est modifiée
